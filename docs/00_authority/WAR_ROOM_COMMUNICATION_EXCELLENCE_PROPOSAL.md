...
## 6. Teams Integration — Interactive War Room Surface

### 6.1 Beyond Command Bridge

The current Command Bridge posts a summary to a management channel. This proposal extends Teams integration for the War Room:

| Capability | Description |
|---|---|
| **War Room Teams Channel** | On activation, optionally auto-create a dedicated Teams channel for this War Room (named `WR-2026-0001`). All structured updates posted there. Channel archived on close. |
| **Approval Cards** | Sub-actions and decisions posted as Adaptive Cards with Approve/Deny/Delegate buttons. Decision flows back to Commander without context switch. |
| **Join War Room from Teams** | Adaptive Card includes "Join War Room" button that registers the user as an observer in Commander. |
| **Status Polling** | Bot can respond to `@Commander status` in the War Room channel with current state summary. |
| **Handoff Cards** | On shift change, outgoing analyst receives a handoff Adaptive Card. Must acknowledge and provide status summary before the system records the handoff. |

### 6.2 Email Integration — Structured War Room Updates

| Capability | Description |
|---|---|
| **Structured update emails** | Templated, sections-based emails following the update template in §4.4. Machine-parseable headers for mail rules. |
| **Reply-to-escalate** | Subscriber replies to an update email with "ESCALATE" → Commander ingests and triggers escalation pathway. |
