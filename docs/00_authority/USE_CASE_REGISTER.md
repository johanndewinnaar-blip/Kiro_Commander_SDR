...
| UC-213 | Measure journey workflow lifecycle (signal to validated outcome) | System | — | — | journey.ts, journey-template.ts, tagger engines, formula engines | SYSTEM | — | Unit 51 | NOT BUILT (data layer built, engines built, read models built) |
| UC-214 | Measure journey tempo per OODA phase | SOM, CISO | operational | — | journey.ts, journey_lifecycle_tempo read model | SYSTEM | — | Unit 51 | NOT BUILT (data layer built, read model schema built) |
| UC-215 | Detect journey leakage (stalled past template threshold) | System | — | — | journey.ts, journey-template.ts, leakage-risk formula, journey_leakage_report read model | SYSTEM | — | Unit 51 | NOT BUILT (data layer built, formula built, read model built) |
| UC-216 | Define journey template (expected workflow pattern) | SOM | operational | — | journey-template.ts, 33 seed templates | SYSTEM | — | Unit 51 | NOT BUILT (entity built, 33 templates seeded) |
| UC-217 | Measure journey quality (formula-driven outcome scoring) | SOM, CISO | operational | — | journey-quality formula, journey_quality_scores read model, strategy policy fixtures | SYSTEM | — | Unit 51 | NOT BUILT (formula built, read model built, policy seeded) |
