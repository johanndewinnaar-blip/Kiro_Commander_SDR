...
## Commander AI commitment

Commander AI is core from product v1.1 onward and is not optional later scope. In product v1.1 it is implemented only as grounded local/mock behaviour, prompt governance and reviewable assistance. Runtime selection remains open until the AWS/Bedrock/AgentCore evaluation lane records a decision.

## Schedule distinction

Kiro pack v1.1 is a corrected conversion-pack version. Product v1.1 is the first product build increment. They are related but not the same object.
...
## v1.2 remediation interpretation

v1.2 is a pack-remediation version, not a product runtime release. It strengthens source coverage, EARS depth and traceability before product build execution. Product implementation remains staged through the Commander build roadmap after validation.
