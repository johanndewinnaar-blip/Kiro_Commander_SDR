...
|---|---|---|
| Microsoft Graph / Entra ID | Source DOCX archived; mock only. | Scope, least privilege, TEST MODE (test tenant), identity data classification. |
...
| AWS general / Route 53 | Source DOCX archived; mock only. | TEST MODE (AWS test account), IAM read-only, asset/DNS/security signal mapping. |
